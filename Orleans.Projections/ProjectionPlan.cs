...
public record ProjectionPlan<TState> (INode[] Nodes)
{
	...
	// todo: usually specific state class is used in the single grain, so maybe it is possible to use the usual dictionary here instead of the concurrent one. But let's keep it concurrent for now, just in case.
